...
```{r exploring-clusters}
whole_data_clusters <- whole_data %>% mutate(cluster = as.factor(kmeans_result$cluster))

summary(whole_data_clusters %>% mutate(wine_colour = as.factor(wine_colour)) %>% filter(cluster == 1))
print('-------------------')
summary(whole_data_clusters %>% mutate(wine_colour = as.factor(wine_colour)) %>% filter(cluster == 2))
print('-------------------')
summary(whole_data_clusters %>% mutate(wine_colour = as.factor(wine_colour)) %>% filter(cluster == 3))


whole_data_clusters %>% 
select(cluster, quality) %>%
mutate(cluster = as.factor(cluster)) %>%
ggplot(aes(cluster, quality)) +
geom_boxplot(aes(fill = cluster)) 


ggplot(whole_data_clusters, aes(x = quality, y = cluster)) + geom_joy()
```

There's a quality difference between groups 1 and the rest, but still the quality range of each cluster is too broad. Finally, I think I'll go on changing the aim of the problem then: predict wine colour.
