# Bayesian blocks using PELT.
# Jan Galkowski, empirical_bayesian@ieee.org
# Begun 28th July 2016, last changed 1st August 2016.
#
...
bayesianBlocksByPeltHistogramFrom<- function(x, add=FALSE, log="", xlab="", ylab="", title="Bayesian Blocks Histogram (via PELT)", 
                                             subtitle="", xlim=NULL, col="blue", lwd=3, fill=NA, alpha=1.0, xout=NA, penalty="SIC",
  ...
  if (!add)
  {
    if (!is.null(xlim))
    {
      plot(X, rep(biggest, length(X)), type="n", xlim=xlim, xlab=xlab, ylab=ylab, log=log, 
           main=sprintf(title, subtitle), ylim=c(0,biggest))
    } else
    {
