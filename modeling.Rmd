...
xgb_with_folds <- function(train_df) {
  tr <- train(y=train_df$quality,
             ...
             preProc=c('center', 'scale'),
             tuneGrid=expand.grid(nrounds = c(1000), lambda = c(0, 0.5, 1), eta = c(0.01, 0.1))
             )
...
error_metrics_xgb <- xgb_with_folds(train_set)
...
* Cluster the observations and apply a model in each cluster (¿maybe one model for white wine and another model for red wine? ¿use clustering models?)
